 ...
 *
 * Los datos se filtran por las marcas configuradas globalmente en la tabla own_brand_brands.
 * No hay asignación por usuario: cualquier own_brand_user ve todas las marcas configuradas.
 */
...
function BrandsManager() {
  ...
  const { data: configuredBrands, isLoading: brandsLoading } = trpc.ownBrand.listBrands.useQuery();
  const { data: allBrands, isLoading: allBrandsLoading } = trpc.ownBrand.listAllBrands.useQuery();

  const addMutation = trpc.ownBrand.addBrand.useMutation({
    onSuccess: () => {
      utils.ownBrand.listBrands.invalidate();
      toast.success("Marca agregada a la configuración de Marca Propia.");
    },
    onError: (e) => toast.error(e.message),
  });

  const removeMutation = trpc.ownBrand.removeBrand.useMutation({
    onSuccess: () => {
      utils.ownBrand.listBrands.invalidate();
      toast.success("Marca removida de la configuración de Marca Propia.");
    },
    onError: (e) => toast.error(e.message),
  });

  const configuredIds = new Set((configuredBrands ?? []).map((b) => b.id));

  const availableBrands = useMemo(() => {
    if (!allBrands) return [];
    const q = brandSearch.toLowerCase();
    return allBrands.filter(
      (b) => !configuredIds.has(b.id) && (!q || b.name.toLowerCase().includes(q))
    );
  }, [allBrands, configuredIds, brandSearch]);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          ...
          Cualquier usuario Marca Propia ve todos los artículos de las marcas aquí configuradas.
        </p>
      </div>
        ...
        <CardContent className="p-0">
          {brandsLoading ? (
            ...
            <p className="text-center text-sm text-muted-foreground py-8">
              No hay marcas configuradas. Agrega marcas desde el panel de abajo.
            </p>
          ) : (
              ...
              {configuredBrands?.map((brand) => (
                <div key={brand.id} className="flex items-center justify-between px-4 py-3">
                  ...
                  <Button
                    ...
                    onClick={() => removeMutation.mutate({ brandId: brand.id })}
        ...
        <CardContent className="space-y-3">
          ...
          {allBrandsLoading ? (
            ...
            <p className="text-center text-sm text-muted-foreground py-4">
              {brandSearch ? "No se encontraron marcas con ese nombre." : "Todas las marcas ya están configuradas."}
            </p>
          ) : (
            <div className="border border-border rounded-md divide-y divide-border max-h-72 overflow-y-auto">
              {availableBrands.map((brand) => (
                <div key={brand.id} className="flex items-center justify-between px-4 py-2.5 hover:bg-muted/30">
                  <div>
                    <p className="text-sm font-medium text-foreground">{brand.name}</p>
                    <p className="text-xs text-muted-foreground font-mono">{brand.id}</p>
                  </div>
                  <Button
                    ...
                    onClick={() => addMutation.mutate({ brandId: brand.id })}
...
export default function OwnBrandPortal() {
  ...
  const { data: branchesForStock } =
    trpc.ownBrand.getBranchesForStock.useQuery(undefined, { enabled: canAccessPortal });

  const { data: branchesForSales } =
    trpc.ownBrand.getBranchesForSales.useQuery(undefined, { enabled: canAccessPortal });
  ...
  const { data: salesByPB, isLoading: salesPBLoading } =
    trpc.ownBrand.getSalesByProductBranch.useQuery({
      ...
      search: salesProductId
        ? brandProducts?.find((p) => p.id === salesProductId)?.name
        : undefined,
  ...
  const handleDownloadCSV = async () => {
    ...
    try {
      ...
      XLSX.writeFile(wb, `marca_propia_ventas_${from}_${to}.xlsx`);
    } finally {
  ...
  const handleDownloadStockCSV = async () => {
    ...
    try {
      ...
      const wsData = [
        ...
        ...rows.map((r) => [
          r.producto, r.int_sku, r.tienda, r.sap_id ?? "",
          r.stock_actual, r.min_stock ?? "",
        ]),
      ];
      const ws = XLSX.utils.aoa_to_sheet(wsData);
      const wb = XLSX.utils.book_new();
  ...
  const { data: dailyDetail, isLoading: dailyDetailLoading } =
    trpc.ownBrand.getSalesDailyDetail.useQuery({
      ...
      <div className="min-h-screen bg-background flex flex-col items-center justify-center gap-6 p-8">
        ...
        <div className="text-center space-y-2 max-w-md">
          ...
          <p className="text-muted-foreground text-sm">
            Esta página es exclusiva para usuarios Marca Propia. Tu perfil no tiene acceso a este portal.
          </p>
        </div>
  ...
  const visibleTabs = TABS_OWN_BRAND.filter(
    (t) => !t.adminOnly || canManageBrands
  );
  ...
  return (
    <div className="min-h-screen bg-background">
          ...
          <>
            <div className="flex flex-wrap items-center gap-3">
              <div className="flex items-center gap-2">
                ...
                <Input type="date" value={from} onChange={(e) => setFrom(e.target.value)} className="w-36 text-sm h-8" />
              </div>
              <div className="flex items-center gap-2">
                <label className="text-sm text-muted-foreground whitespace-nowrap">Hasta</label>
                <Input type="date" value={to} onChange={(e) => setTo(e.target.value)} className="w-36 text-sm h-8" />
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
              ...
              <KpiCard title="Tickets" value={fmt(summary?.total_tickets)} icon={ShoppingCart} color="bg-[var(--ff-cobalto)]" loading={summaryLoading} />
              <KpiCard title="Unidades" value={fmt(summary?.total_unidades)} icon={Package} color="bg-[var(--ff-celeste)]" loading={summaryLoading} />
              <KpiCard title="Artículos Vendidos" value={fmt(summary?.productos_vendidos)} icon={Layers} color="bg-[var(--ff-mostaza)]" loading={summaryLoading} />
              <KpiCard title="Tiendas Activas" value={fmt(summary?.tiendas_activas)} icon={Store} color="bg-[var(--ff-granate)]" loading={summaryLoading} />
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <Card className="border-border/50">
                <CardHeader className="pb-2">
                  <CardTitle className="text-base font-semibold">Tendencia de Ventas Diarias</CardTitle>
                </CardHeader>
                <CardContent>
                  {dailyLoading ? <Skeleton className="h-52 w-full" /> : (
              ...
              <Card className="border-border/50">
                <CardHeader className="pb-2">
                  <CardTitle className="text-base font-semibold">Ventas por Mes (últimos 6 meses)</CardTitle>
                </CardHeader>
                <CardContent>
                  {monthlyLoading ? <Skeleton className="h-52 w-full" /> : (
                    <ResponsiveContainer width="100%" height={210}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <TableBody>
                          {topProducts?.map((p, i) => (
                            <TableRow key={p.int_sku}>
                              ...
                              <TableCell className="text-xs text-right font-medium">{fmtCurrency(p.total_ventas)}</TableCell>
                              <TableCell className="text-xs text-right">{fmt(p.unidades_vendidas)}</TableCell>
                            </TableRow>
                          ))}
                          {!topProducts?.length && (
                            <TableRow>
                              <TableCell colSpan={4} className="text-center text-sm text-muted-foreground py-8">Sin ventas en el período seleccionado</TableCell>
                            </TableRow>
                          )}
                        </TableBody>
                      </Table>
              ...
              <Card className="border-border/50">
                <CardHeader className="pb-2">
                  <CardTitle className="text-base font-semibold">Ventas por Tienda</CardTitle>
                </CardHeader>
                <CardContent>
                  {branchLoading ? <Skeleton className="h-52 w-full" /> : salesByBranch && salesByBranch.length > 0 ? (
                    <ResponsiveContainer width="100%" height={520}>
                      ...
                      >
                        ...
                        <XAxis type="number" tick={{ fontSize: 10, fill: "var(--muted-foreground)" }} tickFormatter={(v) => `S/ ${fmt(v)}`} />
                        <YAxis type="category" dataKey="tienda" tick={{ fontSize: 11, fill: "var(--muted-foreground)" }} width={80} />
                        <Tooltip formatter={(v: number) => [fmtCurrency(v), "Ventas"]} contentStyle={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "0.5rem" }} />
                    ...
                    <div className="flex items-center justify-center h-52 text-muted-foreground text-sm">Sin ventas en el período seleccionado</div>
                  )}
                </CardContent>
              </Card>
            </div>
                      ...
                      {catalogLoading
                        ...
                        : catalogData?.rows.map((p) => (
                            <TableRow key={p.id}>
                              ...
                              <TableCell className="text-right font-medium">{fmt(p.stock_total)}</TableCell>
                              <TableCell className="text-right">{p.tiendas_con_stock}</TableCell>
                              <TableCell>
                                {p.stock_total === 0 ? (
                                  <Badge variant="destructive" className="text-xs">Sin stock</Badge>
                                ) : p.stock_total < 5 ? (
                                  <Badge className="text-xs bg-[var(--ff-mostaza)] text-white">Stock bajo</Badge>
                                ) : (
                                  <Badge className="text-xs bg-[var(--ff-esmeralda)] text-white">En stock</Badge>
                                )}
                              </TableCell>
                            </TableRow>
                          ))}
            ...
            {catalogData && catalogData.total > PAGE_SIZE && (
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">
                  Mostrando {catalogPage * PAGE_SIZE + 1}–{Math.min((catalogPage + 1) * PAGE_SIZE, catalogData.total)} de {catalogData.total}
                </span>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" disabled={catalogPage === 0} onClick={() => setCatalogPage((p) => p - 1)}><ChevronLeft className="h-4 w-4" /></Button>
                  <Button variant="outline" size="sm" disabled={(catalogPage + 1) * PAGE_SIZE >= catalogData.total} onClick={() => setCatalogPage((p) => p + 1)}><ChevronRight className="h-4 w-4" /></Button>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="space-y-4">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="ml-auto flex items-center gap-3">
                ...
                <Button variant="outline" size="sm" className="h-9 gap-1.5" onClick={handleDownloadStockCSV} disabled={isExportingStock || !canAccessPortal}>
                      ...
                      {stockLoading
                        ...
                        : stockData?.rows.map((s, i) => (
                            <TableRow key={i} className={s.stock_actual === 0 ? "opacity-60" : ""}>
                              ...
                              <TableCell className="text-right text-muted-foreground">{s.min_stock != null ? fmt(s.min_stock) : "—"}</TableCell>
                              <TableCell>
                                {s.stock_actual === 0 ? (
                                  <Badge variant="destructive" className="text-xs">Sin stock</Badge>
            ...
            {stockData && stockData.total > PAGE_SIZE && (
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">
                  Mostrando {stockPage * PAGE_SIZE + 1}–{Math.min((stockPage + 1) * PAGE_SIZE, stockData.total)} de {stockData.total}
                </span>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" disabled={stockPage === 0} onClick={() => setStockPage((p) => p - 1)}><ChevronLeft className="h-4 w-4" /></Button>
                  <Button variant="outline" size="sm" disabled={(stockPage + 1) * PAGE_SIZE >= stockData.total} onClick={() => setStockPage((p) => p + 1)}><ChevronRight className="h-4 w-4" /></Button>
                </div>
              </div>
            )}
          </div>
        ...
        {activeTab === "recepciones" && (
          <div className="space-y-4">
                      ...
                      {recLoading
                        ...
                        : receptionsData?.rows.map((r, i) => (
                            <TableRow key={i}>
                              ...
                              <TableCell className="text-right">{fmt(r.ordered_quantity)}</TableCell>
                              <TableCell className="text-right">{r.received_quantity != null ? fmt(r.received_quantity) : "—"}</TableCell>
                              <TableCell>
                                {r.status ? (
                                  <Badge className="text-xs bg-[var(--ff-esmeralda)] text-white">{r.status}</Badge>
                                ) : (
            ...
            {receptionsData && receptionsData.total > PAGE_SIZE && (
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">
                  Mostrando {recPage * PAGE_SIZE + 1}–{Math.min((recPage + 1) * PAGE_SIZE, receptionsData.total)} de {receptionsData.total}
                </span>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" disabled={recPage === 0} onClick={() => setRecPage((p) => p - 1)}><ChevronLeft className="h-4 w-4" /></Button>
                  <Button variant="outline" size="sm" disabled={(recPage + 1) * PAGE_SIZE >= receptionsData.total} onClick={() => setRecPage((p) => p + 1)}><ChevronRight className="h-4 w-4" /></Button>
                </div>
              </div>
            )}
          </div>
        ...
        {activeTab === "ventas" && (
          <div className="space-y-4">
              ...
              <div className="flex flex-col gap-1 flex-1 min-w-[240px] max-w-sm">
                ...
                >
                  ...
                  <SelectContent className="max-h-72">
                    ...
                    {filteredProducts.length === 0 && !brandProductsLoading && (
                      <div className="px-3 py-2 text-xs text-muted-foreground text-center">Sin resultados</div>
                    )}
                  </SelectContent>
                </Select>
            ...
            {salesByPB && salesByPB.total > PAGE_SIZE && (
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">
                  Mostrando {salesPage * PAGE_SIZE + 1}–{Math.min((salesPage + 1) * PAGE_SIZE, salesByPB.total)} de {salesByPB.total}
                </span>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" disabled={salesPage === 0} onClick={() => setSalesPage((p) => p - 1)}><ChevronLeft className="h-4 w-4" /></Button>
                  <Button variant="outline" size="sm" disabled={(salesPage + 1) * PAGE_SIZE >= salesByPB.total} onClick={() => setSalesPage((p) => p + 1)}><ChevronRight className="h-4 w-4" /></Button>
                </div>
              </div>
            )}
          </div>
      ...
      <Dialog open={!!detailModal?.open} onOpenChange={(open) => !open && setDetailModal(null)}>
        ...
        >
          <DialogHeader>
            <DialogTitle className="text-base font-bold uppercase tracking-wide leading-tight" style={{ fontFamily: "'Italian Plate No 1', sans-serif" }}>
          ...
          {dailyDetailLoading ? (
            <div className="space-y-2 py-4">{[...Array(6)].map((_, i) => <Skeleton key={i} className="h-8 w-full" />)}</div>
          ) : !dailyDetail?.length ? (
            <p className="text-center text-sm text-muted-foreground py-8">No hay ventas registradas para este artículo y tienda en el período.</p>
          ) : (
            <>
              <div className="grid grid-cols-3 gap-3 my-2">
                <div className="rounded-lg p-3 text-center" style={{ background: "#0D344A1A" }}>
                  ...
                  <p className="text-xl font-bold" style={{ fontFamily: "'Italian Plate No 1', sans-serif", color: "#0D344A" }}>{dailyDetail.length}</p>
                </div>
              ...
              <div style={{ height: 240 }}>
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <XAxis dataKey="fecha" tick={{ fontSize: 10, fill: "#757471", fontFamily: "'Sailec', sans-serif" }} axisLine={false} tickLine={false} />
                    <YAxis yAxisId="monto" orientation="left" tick={{ fontSize: 10, fill: "#757471", fontFamily: "'Sailec', sans-serif" }} tickFormatter={(v) => `S/${(v / 1000).toFixed(1)}K`} axisLine={false} tickLine={false} width={52} />
                    <YAxis yAxisId="qty" orientation="right" tick={{ fontSize: 10, fill: "#757471", fontFamily: "'Sailec', sans-serif" }} axisLine={false} tickLine={false} width={36} />
                    <Tooltip
                      formatter={(value: number, name: string) => name === "monto" ? [fmtCurrency(value), "Monto"] : [fmt(value), "Cantidad"]}
                      contentStyle={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "0.5rem", fontFamily: "'Sailec', sans-serif", fontSize: 12 }}
                    />
                    <Legend formatter={(value: string) => value === "monto" ? "Monto" : "Cantidad"} wrapperStyle={{ fontSize: 11, fontFamily: "'Sailec', sans-serif" }} />
                    <Line yAxisId="monto" type="monotone" dataKey="monto" stroke="#1A6894" strokeWidth={2} dot={{ r: 3, fill: "#1A6894", strokeWidth: 0 }} activeDot={{ r: 5 }} name="monto" />
                    <Line yAxisId="qty" type="monotone" dataKey="cantidad" stroke="#008064" strokeWidth={2} dot={{ r: 3, fill: "#008064", strokeWidth: 0 }} activeDot={{ r: 5 }} name="cantidad" />
                  </LineChart>
                </ResponsiveContainer>
              </div>
                  ...
                  <TableBody>
                    {dailyDetail.map((r) => (
                      <TableRow key={r.fecha} className="border-border/50">
                        <TableCell className="pl-4 text-sm font-medium">{format(new Date(r.fecha), "dd MMM yyyy", { locale: es })}</TableCell>
                        <TableCell className="text-right tabular-nums">{fmt(r.cantidad)}</TableCell>
                        <TableCell className="text-right tabular-nums font-medium" style={{ color: "#008064" }}>{fmtCurrency(r.monto)}</TableCell>
