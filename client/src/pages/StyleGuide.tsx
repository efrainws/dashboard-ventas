    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-50 bg-background/95 backdrop-blur border-b border-border/50">
        <div className="container flex items-center justify-between h-14">
          ...
          <Badge variant="secondary" className="text-xs">v2.0 · Flora &amp; Fauna Design System</Badge>
        </div>
      </header>
  ...
  <tfoot><tr><td>Total</td><td>S/ 252,757</td></tr></tfoot>
          ...
          <Section id="autenticacion" title="Autenticación">
            <div className="space-y-6">
              <Card>
                <CardHeader style={{ background: "var(--ff-card-header-bg)", borderBottom: "1px solid var(--ff-card-header-border)" }}>
                  <CardTitle>Método de Acceso</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4 text-sm">
                  <p className="text-muted-foreground">El sistema utiliza autenticación local con <strong className="text-foreground">correo electrónico + contraseña</strong>. El campo <em>nombre de usuario</em> fue eliminado como requisito; el email es el único identificador único del usuario en la base de datos.</p>
                  <div className="grid md:grid-cols-2 gap-4">
                    <div className="rounded-lg border border-border/50 p-4 space-y-2">
                      <p className="font-semibold flex items-center gap-1.5"><CheckCircle2 className="h-4 w-4" style={{ color: "#008064" }} /> Flujo de Login</p>
                      <ol className="space-y-1 text-xs text-muted-foreground list-decimal list-inside">
                        <li>El usuario ingresa a <code className="font-mono bg-muted px-1 rounded">/login</code></li>
                        <li>Introduce su <strong className="text-foreground">correo electrónico</strong> y contraseña</li>
                        <li>El servidor valida contra <code className="font-mono bg-muted px-1 rounded">users.email</code> (campo único)</li>
                        <li>Se genera una sesión JWT firmada con <code className="font-mono bg-muted px-1 rounded">JWT_SECRET</code></li>
                        <li>Redirección a <code className="font-mono bg-muted px-1 rounded">/</code> (o al portal del rol correspondiente)</li>
                      </ol>
                    </div>
                    <div className="rounded-lg border border-border/50 p-4 space-y-2">
                      <p className="font-semibold flex items-center gap-1.5"><Info className="h-4 w-4" style={{ color: "#1A6894" }} /> Flujo de Activación</p>
                      <ol className="space-y-1 text-xs text-muted-foreground list-decimal list-inside">
                        <li>El administrador crea el usuario con email + contraseña temporal</li>
                        <li>Se genera un token UUID en <code className="font-mono bg-muted px-1 rounded">activation_tokens</code></li>
                        <li>Se envía un correo con el enlace <code className="font-mono bg-muted px-1 rounded">/activate/:token</code></li>
                        <li>El usuario verifica su email en la página de activación</li>
                        <li>Establece una nueva contraseña y queda habilitado para ingresar</li>
                      </ol>
                    </div>
                  </div>
                  <div className="rounded-lg border border-amber-200 bg-amber-50 dark:bg-amber-950/20 dark:border-amber-800/50 p-3 text-xs text-amber-800 dark:text-amber-300">
                    <p className="font-semibold flex items-center gap-1.5"><AlertTriangle className="h-3.5 w-3.5" /> Nota importante sobre contraseñas</p>
                    <p className="mt-1">La contraseña temporal asignada por el administrador <strong>no se envía por correo</strong>. El administrador debe comunicarla al usuario por otro canal (teléfono, mensaje directo). El correo solo contiene el enlace de activación.</p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardHeader style={{ background: "var(--ff-card-header-bg)", borderBottom: "1px solid var(--ff-card-header-border)" }}>
                  <CardTitle>Tokens de Activación</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3 text-sm">
                  <p className="text-muted-foreground">Los tokens se almacenan en la tabla <code className="font-mono text-xs bg-muted px-1 py-0.5 rounded">activation_tokens</code> y tienen una vigencia de <strong className="text-foreground">48 horas</strong>. Un administrador con permisos puede regenerar el token y reenviar el correo desde la página de Gestión de Usuarios.</p>
                  <table className="ff-table">
                    <thead><tr><th className="text-left">Campo</th><th className="text-left">Tipo</th><th className="text-left">Descripción</th></tr></thead>
                    <tbody>
                      <tr><td className="font-mono text-xs text-left">token</td><td className="text-left">varchar(128) UNIQUE</td><td className="text-left">UUID v4 enviado en el enlace de activación</td></tr>
                      <tr><td className="font-mono text-xs text-left">user_id</td><td className="text-left">int</td><td className="text-left">FK a users.id</td></tr>
                      <tr><td className="font-mono text-xs text-left">email</td><td className="text-left">varchar(320)</td><td className="text-left">Email del usuario para verificación en la página</td></tr>
                      <tr><td className="font-mono text-xs text-left">expires_at</td><td className="text-left">timestamp</td><td className="text-left">Expiración (48 h desde creación)</td></tr>
                      <tr><td className="font-mono text-xs text-left">used</td><td className="text-left">int (0/1)</td><td className="text-left">1 = token ya utilizado</td></tr>
                    </tbody>
                  </table>
                </CardContent>
              </Card>
          ...
          {/* ══ ROLES & PERMISOS ═══════════════════════════════════════════════════════════ */}
          <Section id="roles" title="Roles & Permisos">
            <div className="space-y-6">
              <p className="text-sm text-muted-foreground">El sistema define <strong className="text-foreground">7 roles de usuario</strong> con diferentes niveles de acceso. El rol se almacena en <code className="font-mono text-xs bg-muted px-1 py-0.5 rounded">users.role</code> y se verifica en cada procedimiento tRPC protegido mediante <code className="font-mono text-xs bg-muted px-1 py-0.5 rounded">ctx.user.role</code>.</p>
              <Card>
                <CardHeader style={{ background: "var(--ff-card-header-bg)", borderBottom: "1px solid var(--ff-card-header-border)" }}>
                    ...
                    <table className="ff-table">
                      <thead>
                        <tr>
                          ...
                          <th className="text-left">Acceso a datos</th>
                          <th className="text-left">Puede crear usuarios</th>
                          <th className="text-left">Portal exclusivo</th>
                        </tr>
                      </thead>
                      <tbody>
                        {[
                          { role: "system_specialist", label: "Especialista de Sistemas", data: "Sin restricciones", create: "Todos los roles", portal: "—" },
                          { role: "operations_specialist", label: "Especialista de Operaciones", data: "Sin restricciones", create: "Solo store_user", portal: "—" },
                          { role: "cst_user", label: "Usuario CST", data: "Sin restricciones", create: "Solo store_user", portal: "—" },
                          { role: "commercial_specialist", label: "Especialista Comercial", data: "Sin restricciones", create: "Solo supplier_user", portal: "Portales Adicionales" },
                          { role: "store_user", label: "Usuario Tienda", data: "Solo su tienda asignada", create: "No puede", portal: "—" },
                          { role: "supplier_user", label: "Usuario Proveedor", data: "Solo su proveedor asignado", create: "No puede", portal: "/supplier" },
                          { role: "own_brand_user", label: "Usuario Marca Propia", data: "Marcas propias configuradas", create: "No puede", portal: "/marca-propia" },
                        ].map((r) => (
                          <tr key={r.role}>
                            <td className="font-mono text-xs text-left">{r.role}</td>
                            <td className="text-left">{r.label}</td>
                            <td className="text-left text-muted-foreground text-xs">{r.data}</td>
                            <td className="text-left text-muted-foreground text-xs">{r.create}</td>
                            <td className="text-left text-muted-foreground text-xs">{r.portal}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              <Card>
                <CardHeader style={{ background: "var(--ff-card-header-bg)", borderBottom: "1px solid var(--ff-card-header-border)" }}>
                  <CardTitle>Guards de Ruta</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3 text-sm">
                  <p className="text-muted-foreground">Las rutas protegidas usan el componente <code className="font-mono text-xs bg-muted px-1 py-0.5 rounded">ProtectedRoute</code> con un parámetro <code className="font-mono text-xs bg-muted px-1 py-0.5 rounded">guard</code> que define el nivel de acceso requerido.</p>
                  <table className="ff-table">
                    <thead><tr><th className="text-left">Guard</th><th className="text-left">Roles permitidos</th><th className="text-left">Ejemplo de ruta</th></tr></thead>
                    <tbody>
                      <tr><td className="font-mono text-xs text-left">no_supplier</td><td className="text-left text-xs">Todos excepto supplier_user</td><td className="font-mono text-xs text-left">/</td></tr>
                      <tr><td className="font-mono text-xs text-left">managers_only</td><td className="text-left text-xs">system_specialist, cst_user, commercial_specialist</td><td className="font-mono text-xs text-left">/monitoreo-proveedores</td></tr>
                      <tr><td className="font-mono text-xs text-left">system_specialist_only</td><td className="text-left text-xs">system_specialist, operations_specialist</td><td className="font-mono text-xs text-left">/admin/users</td></tr>
                      <tr><td className="font-mono text-xs text-left">system_specialist_strict</td><td className="text-left text-xs">Solo system_specialist</td><td className="font-mono text-xs text-left">/admin/db-connections</td></tr>
                      <tr><td className="font-mono text-xs text-left">own_brand_only</td><td className="text-left text-xs">own_brand_user, system_specialist, admin, commercial_specialist</td><td className="font-mono text-xs text-left">/marca-propia</td></tr>
                    </tbody>
                  </table>
                </CardContent>
              </Card>
          ...
          {/* ══ NAVEGACIÓN ════════════════════════════════════════════════════════════════ */}
          <Section id="navegacion" title="Navegación">
            <div className="space-y-6">
              <p className="text-sm text-muted-foreground">La barra de navegación principal (<code className="font-mono text-xs bg-muted px-1 py-0.5 rounded">NavigationMenu.tsx</code>) es <strong className="text-foreground">sticky</strong>, adaptativa al tema (logo claro/oscuro) y responsiva con menú hamburguesa en móvil. Los ítems visibles varían según el rol del usuario autenticado.</p>
              ...
              <Card>
                ...
                <CardContent>
                  <table className="ff-table">
                    <thead><tr><th className="text-left">Sección</th><th className="text-left">Ruta / Submenu</th><th className="text-left">Roles que lo ven</th></tr></thead>
                    <tbody>
                      <tr><td className="text-left">Inicio</td><td className="font-mono text-xs text-left">/</td><td className="text-left text-xs">Todos (excepto supplier_user y own_brand_user)</td></tr>
                      <tr><td className="text-left">Ventas ▾</td><td className="text-left text-xs">/sales, /hourly, /sales-vs-target, /identified-transactions, /top-products</td><td className="text-left text-xs">Todos (excepto supplier_user y own_brand_user)</td></tr>
                      <tr><td className="text-left">Tickets</td><td className="font-mono text-xs text-left">/tickets</td><td className="text-left text-xs">Todos (excepto supplier_user y own_brand_user)</td></tr>
                      <tr><td className="text-left">Portales Adicionales ▾</td><td className="text-left text-xs">/supplier, /monitoreo-proveedores, /afiliacion, /marca-propia</td><td className="text-left text-xs">system_specialist, commercial_specialist, admin, own_brand_user</td></tr>
                      <tr><td className="text-left">Menú de usuario ▾</td><td className="text-left text-xs">Gestión de Usuarios, Conexiones BD, Cerrar Sesión</td><td className="text-left text-xs">Según rol (ver sección Roles)</td></tr>
                    </tbody>
                  </table>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardHeader style={{ background: "var(--ff-card-header-bg)", borderBottom: "1px solid var(--ff-card-header-border)" }}>
                  <CardTitle>Logos por Tema</CardTitle>
                </CardHeader>
                <CardContent className="text-sm text-muted-foreground space-y-2">
                  <p>La barra de navegación detecta el tema activo y cambia el logo automáticamente:</p>
                  <table className="ff-table">
                    <thead><tr><th className="text-left">Tema</th><th className="text-left">Archivo</th><th className="text-left">Descripción</th></tr></thead>
                    <tbody>
                      <tr><td className="text-left">Light</td><td className="font-mono text-xs text-left">/Logonegro.svg</td><td className="text-left text-xs">Logo en negro para fondos claros (hueso/blanco)</td></tr>
                      <tr><td className="text-left">Dark</td><td className="font-mono text-xs text-left">/Logoclarochico.svg</td><td className="text-left text-xs">Logo en blanco/claro para fondos oscuros</td></tr>
                    </tbody>
                  </table>
                  <p className="text-xs">La lógica usa <code className="font-mono bg-muted px-1 rounded">effectiveTheme</code> del <code className="font-mono bg-muted px-1 rounded">ThemeContext</code>, que resuelve el tema del sistema cuando el usuario elige "Automático".</p>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <table className="ff-table">
                    <thead><tr><th className="text-left">Ruta</th><th className="text-left">Componente</th><th className="text-left">Guard</th><th className="text-left">Descripción</th></tr></thead>
                    <tbody>
                      {[
                        { path: "/login", comp: "Login", guard: "Pública", desc: "Formulario de acceso con email + contraseña" },
                        { path: "/activate/:token", comp: "ActivateAccount", guard: "Pública", desc: "Activación de cuenta nueva y cambio de contraseña" },
                        { path: "/style-guide", comp: "StyleGuide", guard: "Pública", desc: "Esta página" },
                        { path: "/", comp: "Home", guard: "no_supplier", desc: "Página de inicio con módulos de análisis" },
                        { path: "/sales", comp: "SalesByCategory", guard: "no_supplier", desc: "Análisis general de ventas por categoría" },
                        { path: "/hourly", comp: "HourlyAnalysis", guard: "no_supplier", desc: "Análisis de ventas por hora del día" },
                        { path: "/sales-vs-target", comp: "SalesVsTarget", guard: "no_supplier", desc: "Cumplimiento de metas mensuales por tienda" },
                        { path: "/identified-transactions", comp: "IdentifiedTransactions", guard: "no_supplier", desc: "Transacciones con cliente identificado" },
                        { path: "/top-products", comp: "TopProducts", guard: "no_supplier", desc: "Top 50 productos más vendidos" },
                        { path: "/tickets", comp: "DiscrepancyTickets", guard: "no_supplier", desc: "Reporte y seguimiento de discrepancias" },
                        { path: "/supplier", comp: "SupplierPortal", guard: "supplier_user", desc: "Portal exclusivo de ventas para proveedores" },
                        { path: "/monitoreo-proveedores", comp: "SupplierMonitor", guard: "managers_only", desc: "Administración de accesos de proveedores" },
                        { path: "/afiliacion", comp: "AffiliationReport", guard: "managers_only", desc: "Reporte de afiliación de proveedores" },
                        { path: "/marca-propia", comp: "OwnBrandPortal", guard: "own_brand_only", desc: "Portal de ventas de marca propia" },
                        { path: "/terminos", comp: "TermsPage", guard: "Autenticado", desc: "Términos y condiciones del servicio" },
                        { path: "/acceso-vencido", comp: "AccessExpiredPage", guard: "Autenticado", desc: "Página de acceso vencido para proveedores" },
                        { path: "/admin/users", comp: "UserManagement", guard: "system_specialist_only", desc: "Gestión de usuarios del sistema" },
                        { path: "/admin/db-connections", comp: "DatabaseConnections", guard: "system_specialist_strict", desc: "Configuración de conexiones PostgreSQL" },
                      ].map((r) => (
                        <tr key={r.path}>
                          <td className="font-mono text-xs text-left">{r.path}</td>
                          <td className="font-mono text-xs text-left">{r.comp}</td>
                          <td className="text-left text-xs text-muted-foreground">{r.guard}</td>
                          <td className="text-left text-xs text-muted-foreground">{r.desc}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          <Section id="componentes" title="Componentes Reutilizables">
            <div className="space-y-6">
              <p className="text-sm text-muted-foreground">El proyecto incluye componentes especializados en <code className="font-mono text-xs bg-muted px-1 py-0.5 rounded">client/src/components/</code> que encapsulan lógica de negocio y estilos del sistema de diseño. Antes de crear nuevos componentes, verificar si alguno de los siguientes ya cubre el caso de uso.</p>
              ...
              <Card>
                ...
                <CardContent>
                  <table className="ff-table">
                    <thead><tr><th className="text-left">Componente</th><th className="text-left">Archivo</th><th className="text-left">Descripción</th></tr></thead>
                    <tbody>
                      {[
                        { name: "NavigationMenu", file: "NavigationMenu.tsx", desc: "Barra de navegación principal con soporte de roles, tema y menú móvil" },
                        { name: "KPICard", file: "KPICard.tsx", desc: "Tarjeta de métrica con valor, comparación vs período anterior e indicador de tendencia" },
                        { name: "ProgressRing", file: "ProgressRing.tsx", desc: "Anillo SVG de progreso con semáforo de colores (granate/mostaza/cobalto/esmeralda)" },
                        { name: "DashboardFilters", file: "DashboardFilters.tsx", desc: "Panel de filtros de fecha, sucursal y categoría compartido entre módulos de ventas" },
                        { name: "SalesLineChart", file: "SalesLineChart.tsx", desc: "Gráfico de líneas de ventas con toggle día/mes usando Recharts" },
                        { name: "CategoryPieChart", file: "CategoryPieChart.tsx", desc: "Gráfico de torta de distribución por categoría abuelo" },
                        { name: "BranchBarChart", file: "BranchBarChart.tsx", desc: "Gráfico de barras de ventas por sucursal" },
                        { name: "HourlyLineChart", file: "HourlyLineChart.tsx", desc: "Gráfico de líneas de ventas por hora del día" },
                        { name: "ChannelBreakdown", file: "ChannelBreakdown.tsx", desc: "Desglose de ventas por canal (Presencial / eCommerce / Rappi)" },
                        { name: "StoreTargetCard", file: "StoreTargetCard.tsx", desc: "Tarjeta de cumplimiento de meta por tienda con barra de progreso" },
                        { name: "SortableTableHead", file: "SortableTableHead.tsx", desc: "Encabezado de tabla con soporte de ordenamiento ascendente/descendente" },
                        { name: "ReportDiscrepancyButton", file: "ReportDiscrepancyButton.tsx", desc: "Botón flotante para reportar discrepancias desde cualquier módulo" },
                        { name: "ReportDiscrepancyModal", file: "ReportDiscrepancyModal.tsx", desc: "Modal de reporte de discrepancias con formulario completo" },
                        { name: "TrialPopup", file: "TrialPopup.tsx", desc: "Popup diario para proveedores en trial con días restantes y acceso a T&C" },
                        { name: "AccessDenied", file: "AccessDenied.tsx", desc: "Pantalla de acceso denegado para rutas sin permiso" },
                        { name: "MultiProductSelect", file: "MultiProductSelect.tsx", desc: "Selector múltiple de productos con búsqueda" },
                        { name: "TargetEditModal", file: "TargetEditModal.tsx", desc: "Modal de edición de metas mensuales por tienda" },
                      ].map((c) => (
                        <tr key={c.name}>
                          <td className="font-mono text-xs text-left">{c.name}</td>
                          <td className="font-mono text-xs text-left text-muted-foreground">{c.file}</td>
                          <td className="text-left text-xs text-muted-foreground">{c.desc}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <Card>
                  <CardHeader style={{ background: "var(--ff-card-header-bg)", borderBottom: "1px solid var(--ff-card-header-border)" }}>
                    <CardTitle>KPICard — Ejemplo</CardTitle>
                  </CardHeader>
                  <CardContent className="pt-4 space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                      {[
                        { title: "Ventas Totales", value: 284320, prev: 253100, format: "currency" as const },
                        { title: "Transacciones", value: 18432, prev: 16980, format: "number" as const },
                      ].map((k) => {
                          ...
                          <Card key={k.title} className="border-border/50">
                            <CardContent className="pt-3 pb-2 px-3">
                              ...
                              <p className="text-lg font-bold tabular-nums mt-0.5">
                                {k.format === "currency" ? `S/ ${k.value.toLocaleString("es-PE")}` : k.value.toLocaleString("es-PE")}
                              </p>
                              <p className={`text-[10px] mt-1 flex items-center gap-0.5 ${isPos ? "" : ""}`} style={{ color: isPos ? "#008064" : "#BC2C46" }}>
                                {isPos ? <TrendingUp className="h-2.5 w-2.5" /> : <TrendingDown className="h-2.5 w-2.5" />}
                                {isPos ? "+" : ""}{change.toFixed(1)}% vs período anterior
                              </p>
                            </CardContent>
                          </Card>
                        );
                    ...
                    <p className="text-xs text-muted-foreground">Usar <code className="font-mono bg-muted px-1 rounded">format="currency"</code> para PEN, <code className="font-mono bg-muted px-1 rounded">format="number"</code> para enteros, <code className="font-mono bg-muted px-1 rounded">format="percent"</code> para porcentajes.</p>
                  </CardContent>
                </Card>

                <Card>
                  <CardHeader style={{ background: "var(--ff-card-header-bg)", borderBottom: "1px solid var(--ff-card-header-border)" }}>
                    <CardTitle>ProgressRing — Ejemplo</CardTitle>
                  </CardHeader>
                  <CardContent className="pt-4">
                    <div className="flex items-center justify-around">
                      {[62, 83, 97, 112].map((pct) => {
                        const color = pct < 75 ? "#BC2C46" : pct < 90 ? "#C49705" : pct < 100 ? "#1A6894" : "#008064";
                        const r = 40, sw = 6, circ = 2 * Math.PI * r;
                        const offset = circ - (Math.min(pct, 100) / 100) * circ;
                        return (
                          <div key={pct} className="flex flex-col items-center gap-1">
                            <div className="relative inline-flex items-center justify-center">
                              <svg width={96} height={96} className="-rotate-90">
                                <circle cx={48} cy={48} r={r} fill="none" stroke="var(--muted)" strokeWidth={sw} />
                            ...
                            <span className="text-[10px] text-muted-foreground">Tienda {["A","B","C","D"][[62,83,97,112].indexOf(pct)]}</span>
                          </div>
                        );
                      })}
                    </div>
                    <p className="text-xs text-muted-foreground mt-3">El color cambia automáticamente según el semafóro de cumplimiento (ver sección Indicadores).</p>
                  </CardContent>
                </Card>
              </div>
            </div>
          ...
          {/* ══ FLUJOS DE USUARIO ════════════════════════════════════════════════════════════ */}
          <Section id="flujos" title="Flujos de Usuario">
            <div className="space-y-6">
              <div className="grid md:grid-cols-2 gap-6">
                <Card>
                  <CardHeader style={{ background: "var(--ff-card-header-bg)", borderBottom: "1px solid var(--ff-card-header-border)" }}>
                  ...
                  <CardContent className="text-sm space-y-3">
                    <p className="text-muted-foreground">Solo los roles con permisos de creación pueden acceder al formulario. El flujo es el siguiente:</p>
                    <ol className="space-y-2 text-xs text-muted-foreground list-decimal list-inside">
                      <li>Administrador abre el diálogo desde <strong className="text-foreground">/admin/users</strong></li>
                      <li>Completa: <strong className="text-foreground">nombre, email, contraseña temporal, rol</strong> (y tienda o proveedor según el rol)</li>
                      <li>El sistema crea el usuario y genera un token de activación</li>
                      <li>Si el email es válido, se envía automáticamente el correo con el enlace <code className="font-mono bg-muted px-1 rounded">/activate/:token</code></li>
                      <li>El administrador comunica la contraseña temporal por otro canal (la contraseña NO se envía por correo)</li>
                      <li>El usuario activa su cuenta, establece nueva contraseña e inicia sesión</li>
                    </ol>
                    <div className="rounded-lg border border-border/50 p-3 text-xs">
                      <p className="font-semibold text-foreground mb-1">Reglas de creación por rol:</p>
                      <ul className="space-y-0.5 text-muted-foreground">
                        <li>• <strong className="text-foreground">system_specialist</strong>: puede crear cualquier rol</li>
                        <li>• <strong className="text-foreground">operations_specialist</strong>: solo store_user</li>
                        <li>• <strong className="text-foreground">cst_user</strong>: solo store_user</li>
                        <li>• <strong className="text-foreground">commercial_specialist</strong>: solo supplier_user</li>
                      </ul>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card>
                  <CardHeader style={{ background: "var(--ff-card-header-bg)", borderBottom: "1px solid var(--ff-card-header-border)" }}>
                    <CardTitle>Flujo de Proveedores</CardTitle>
                  </CardHeader>
                  <CardContent className="text-sm space-y-3">
                    <p className="text-muted-foreground">Los usuarios con rol <code className="font-mono text-xs bg-muted px-1 py-0.5 rounded">supplier_user</code> tienen un ciclo de vida específico controlado por el campo <code className="font-mono text-xs bg-muted px-1 py-0.5 rounded">supplier_status</code>:</p>
                    <table className="ff-table">
                      <thead><tr><th className="text-left">Estado</th><th className="text-left">Descripción</th></tr></thead>
                      <tbody>
                        {[
                          { s: "pending_activation", d: "Cuenta creada, pendiente de activar por el usuario" },
                          { s: "trial_active", d: "Trial de 7 días activo desde la activación" },
                          { s: "trial_expired", d: "Trial vencido sin aceptar T&C" },
                          { s: "subscribed_active", d: "Suscripción activa (facturada)" },
                          { s: "access_requested", d: "Venció trial, aceptó T&C, pendiente de aprobación" },
                          { s: "suspended", d: "Acceso suspendido manualmente" },
                        ].map((r) => (
                          <tr key={r.s}>
                            <td className="font-mono text-xs text-left">{r.s}</td>
                            <td className="text-left text-xs text-muted-foreground">{r.d}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                    <p className="text-xs text-muted-foreground">El <strong className="text-foreground">TrialPopup</strong> se muestra una vez por día calendario cuando el estado es <code className="font-mono bg-muted px-1 rounded">trial_active</code>, informando los días restantes.</p>
                  </CardContent>
                </Card>
              </div>
              ...
              <Card>
                <CardHeader style={{ background: "var(--ff-card-header-bg)", borderBottom: "1px solid var(--ff-card-header-border)" }}>
                  <CardTitle>Redirecciones Automáticas por Rol</CardTitle>
                </CardHeader>
                <CardContent className="text-sm text-muted-foreground space-y-2">
                  <p>Al iniciar sesión, el sistema redirige automáticamente al portal correspondiente según el rol:</p>
                  <table className="ff-table">
                    <thead><tr><th className="text-left">Rol</th><th className="text-left">Redirección automática</th><th className="text-left">Razón</th></tr></thead>
                    <tbody>
                      <tr><td className="font-mono text-xs text-left">supplier_user</td><td className="font-mono text-xs text-left">/supplier</td><td className="text-left text-xs">Portal exclusivo de proveedores</td></tr>
                      <tr><td className="font-mono text-xs text-left">own_brand_user</td><td className="font-mono text-xs text-left">/marca-propia</td><td className="text-left text-xs">Portal exclusivo de marca propia</td></tr>
                      <tr><td className="font-mono text-xs text-left">Resto de roles</td><td className="font-mono text-xs text-left">/</td><td className="text-left text-xs">Página de inicio con módulos de análisis</td></tr>
                    </tbody>
                  </table>
                </CardContent>
              </Card>
          ...
          {/* ══ BASE DE DATOS ════════════════════════════════════════════════════════════════ */}
          <Section id="bd" title="Base de Datos">
            <div className="space-y-6">
              <p className="text-sm text-muted-foreground">El sistema usa <strong className="text-foreground">dos bases de datos</strong>: una MySQL local (Drizzle ORM) para usuarios y configuración, y una o más PostgreSQL externas (vía conexiones configurables) para los datos de ventas e inventario.</p>

              <Card>
                <CardHeader style={{ background: "var(--ff-card-header-bg)", borderBottom: "1px solid var(--ff-card-header-border)" }}>
                  <CardTitle>Tablas MySQL (Base de Datos Local)</CardTitle>
                </CardHeader>
                <CardContent>
                  <table className="ff-table">
                    <thead><tr><th className="text-left">Tabla</th><th className="text-left">Propósito</th><th className="text-left">Campos clave</th></tr></thead>
                    <tbody>
                      {[
                        { t: "users", p: "Usuarios del sistema", f: "id, email (UNIQUE), password, name, role, assignedStoreCode, assignedSupplierId, supplierStatus" },
                        { t: "activation_tokens", p: "Tokens de activación de cuentas nuevas", f: "token (UNIQUE), user_id, email, expires_at, used" },
                        { t: "store_monthly_targets", p: "Metas mensuales de ventas por tienda", f: "month (YYYY-MM), store_id, monthly_target_amount" },
                        { t: "discrepancy_tickets", p: "Reportes de discrepancias en datos", f: "module, date_from, date_to, store_id, priority, status, reported_by_id" },
                        { t: "terms_versions", p: "Versiones de Términos y Condiciones", f: "version, content, is_active" },
                        { t: "terms_acceptance", p: "Registro de aceptaciones de T&C por usuario", f: "user_id, terms_version_id, accepted_at, ip" },
                        { t: "own_brand_brands", p: "Marcas configuradas como Marca Propia", f: "brand_id (UUID de PostgreSQL, UNIQUE)" },
                        { t: "db_connections", p: "Conexiones a PostgreSQL externas", f: "host, port, database, username, password_encrypted, ssl_enabled, purpose, is_active" },
                      ].map((r) => (
                        <tr key={r.t}>
                          <td className="font-mono text-xs text-left">{r.t}</td>
                          <td className="text-left text-xs">{r.p}</td>
                          <td className="text-left text-xs text-muted-foreground">{r.f}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <Card>
                <CardHeader style={{ background: "var(--ff-card-header-bg)", borderBottom: "1px solid var(--ff-card-header-border)" }}>
                  <CardTitle>Conexiones PostgreSQL Externas</CardTitle>
                </CardHeader>
                <CardContent className="text-sm space-y-3">
                  <p className="text-muted-foreground">Las conexiones a PostgreSQL se gestionan dinámicamente desde la página <code className="font-mono text-xs bg-muted px-1 py-0.5 rounded">/admin/db-connections</code> (solo <strong className="text-foreground">system_specialist</strong>). Las contraseñas se almacenan cifradas con AES-256.</p>
                  <div className="grid md:grid-cols-2 gap-4">
                    <div className="rounded-lg border border-border/50 p-3 space-y-1.5 text-xs">
                      <p className="font-semibold text-foreground">Propósitos de conexión</p>
                      <ul className="space-y-0.5 text-muted-foreground">
                        <li>• <strong className="text-foreground">sales</strong>: datos de ventas (sales_header, sales_detail)</li>
                        <li>• <strong className="text-foreground">stock</strong>: datos de inventario</li>
                        <li>• <strong className="text-foreground">both</strong>: ventas e inventario en la misma BD</li>
                        <li>• <strong className="text-foreground">other</strong>: uso genérico</li>
                      </ul>
                    </div>
                    <div className="rounded-lg border border-border/50 p-3 space-y-1.5 text-xs">
                      <p className="font-semibold text-foreground">Modos SSL</p>
                      <ul className="space-y-0.5 text-muted-foreground">
                        <li>• <strong className="text-foreground">disable</strong>: sin SSL (solo desarrollo local)</li>
                        <li>• <strong className="text-foreground">require</strong>: SSL requerido (default)</li>
                        <li>• <strong className="text-foreground">verify-ca</strong>: verificar CA del servidor</li>
                        <li>• <strong className="text-foreground">verify-full</strong>: verificación completa del certificado</li>
                      </ul>
                    </div>
                  </div>
                  <div className="rounded-lg border border-border/50 p-3 text-xs">
                    <p className="font-semibold text-foreground mb-1">Consultas PostgreSQL — Restricciones</p>
                    <p className="text-muted-foreground">Todas las consultas a PostgreSQL son <strong className="text-foreground">estrictamente de solo lectura</strong> (SELECT). Las operaciones de escritura, actualización o eliminación están prohibidas. Las consultas deben estar optimizadas para manejar millones de registros; usar CTEs, índices y límites de página cuando sea posible.</p>
                  </div>
                </CardContent>
              </Card>
            </div>
          ...
          <footer className="border-t border-border/50 pt-8 pb-4 text-center text-xs text-muted-foreground">
            <p>Flora &amp; Fauna Design System · Versión 2.0 · Uso interno</p>
